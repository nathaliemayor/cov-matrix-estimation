...
portfolio_an_ret <- function(lambda,ret){
  SampleCov<- rags2ridges::covML(ret[1:12,] %>% as.matrix()) 
  precision_m_test<-rags2ridges::ridgeP(SampleCov,lambda = lambda,type = "Alt",target=diag(diag(SampleCov)))
...
## returns w: a 288 x 50 matrix (number of rolling window x number of assets)
portfolio_weights_r2r <- function(ret, lamb){
  w <- matrix(rep(0,dim(ret)[2]*288),nrow = 288)
  onev <- rep(1,dim(ret)[2])
  for (i in 1:288) {
    SampleCov <- rags2ridges::covML(ret[(21*i-20):(252+21*i-21),] %>% as.matrix())
    precision <-rags2ridges::ridgeP(SampleCov,lambda = lamb,type = "Alt",target=diag(diag(SampleCov)))
...
portfolio_ret_r2r <- function(ret, ret_day, a,lamb){
  ptf_r <- 0
  ptf_r <- sum(portfolio_weights_r2r(ret_day,lamb)[a,]*ret[a,])
  return(ptf_r)
}

portfolio_ret_r2r(stock_log_returns_monthly_50[,-51], stock_log_returns_daily_50[,-51],1,1)
sum(lambopt[,2])/24

best_lambda <- function(ret, ret_day){
  optimal_lamb <- matrix(rep(0,2*288),nrow = 288)
  for (b in 1:288) {
    optimal_lamb[b,] <- optimise(portfolio_ret_r2r,interval=c(0.00001,500),tol = 0.01,ret=ret, ret_day=ret_day,a=b,maximum = T)%>%unlist()
  }
  return(optimal_lamb)
}

tic()
opt<- best_lambda(stock_log_returns_monthly_50[,-51], stock_log_returns_daily_50[,-51])
lambopt<-unlist(opt) %>% round(5)
lambopt[,2] %>% mean()*12
lambopt[,2] %>% sd()*sqrt(12)
toc()
